...
for (ct in celltypes){
  ...
  merge$seq_center <- substr(merge$DCP_ID, 1, 6)

  pcs <- c('pPC1', 'pPC2', 'pPC3', 'pPC4', 'pPC5', 'pPC6', 'pPC7', 'pPC8')
  for (n in 1:8){
    nPC <- pcs[1:n]
    write.table(merge[, c('V1', 'sex', 'age', nPC, 'gPC1', 'gPC2', 'gPC3', 'gPC4', 'gPC5')], paste0('/ebs2/aida-sqtl/FreezeV1_0/31_DS_pseudobulk_leafcutter_include_splicing_PC/groups_file.sex.', n, 'PC.', ct, '.txt'), quote=F, row.names=F, col.names=F, sep='\t')
  }
}
